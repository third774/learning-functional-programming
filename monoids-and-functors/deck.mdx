...
There is a pretty small set of these core concepts that can get you really far — similar to how map/filter/reduce make up a core of higher order functions that can get you pretty far. So — once we learn the core type classes, you'll see that these will cover most of the patterns you will see within programming.
...
So — many of the things that we're going to talk about (Monoids included) — you can see parallels to things in mathematics that you may be familiar with.

A lot of these structures were extracted from things that we understood fairly well, like aritmatic — and then once made abstract and generalized it was fairly obvious that it also applies to this thing — and that thing — and that thing.
...
So — Category Theory arose to describe the patterns seen in abstract mathematics and then people who were into Category Theory noticed that some of the things they were doing in programming were patterns they had already seen before. FP then became the effort to see how far they could take that.
```
...
```notes
So - this is an example of category theorists looking at addition, subtraction, multiplication and division — studying basic arithmatic — and identifying something that addition and multiplication have in common with each other and abstracting that out and giving it a name. Then down the road - people in programming who were also into category theory recognizing that this is much broader than just math. These concepts all started as very concrete ideas, and were gradually boiled down and abstracted to be much more generally useful.
```
...
How about the identity value for the All type for booleans we saw earlier? What do you think that is?

And the Any boolean type?
```
...
```notes
So, you can see here that True is the identity value for All type. If you combine it with a False you get back a False. If you combine it with a True you get back True.
...
We can safely assume the initial value to be the identity value for that monoid since appending against it will not change the values produced by the append.

The same cannot be said for Semigroups.
```
